...
export class GB_mapper_MBC3 implements GB_mapper {
    ...
    CPU_read(addr: u32, val: u32): u32 {
        ...
        if (addr < 0x4000) // ROM lo bank
            return unchecked(this.ROM[(addr & 0x3FFF) + this.ROM_bank_offset_lo]);
        if (addr < 0x8000) // ROM hi bank
            return unchecked(this.ROM[(addr & 0x3FFF) + this.ROM_bank_offset_hi]);
        ...
        if (addr < 0xFF00) // OAM
            return unchecked(this.bus.ppu!.read_OAM(addr));
    ...
    CPU_write(addr: u32, val: u32): void {
        ...
        if (addr < 0xC000) { // cart RAM
            ...
            if (this.regs.RAM_bank < 4)
                unchecked(this.cartRAM[(addr & 0x1FFF) + this.RAM_bank_offset] = <u8>val);
            return;
        }
